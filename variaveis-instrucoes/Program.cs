...
namespace variaveis_instrucoes {
  class Program {
    static void Declaracoes() { // tipos de declarações realizadas e suas saídas
      int a;
      int b = 2, c = 3;
    ...
    static void InstrucaoIf(string[] args) { // args = argumento - instruções if else
      if (args.Length == 0) { // se não recebe nenhum argumento, executa o Console.WriteLine("Nenhum argumento");
        Console.WriteLine("Nenhum argumento");
      }
      else if (args.Length == 1) { // se recebe um argumento, executa o Console.WriteLine("Um argumento");
        Console.WriteLine("Um argumento");
      }
      else { // executa se nenhuma das condições anteriores forem falsas (não forem verdadeiras)
        Console.WriteLine($"{args.Length} argumentos");
      }
    }

    static void InstrucaoSwitch(string[] args) { // instruções switch case
    ...
    static void InstrucaoWhile(string[] args) { // instruções de repetição while
      int i = 0;

      while (i < args.Length) { // vai executar o Console.WriteLine(args[i]); enquanto args.Length for menor que i
        Console.WriteLine(args[i]);
        i++; // somando 1 no i cada vez que é executado
      }
    }

    static void InstrucaoDo(string[] args) { // instruções de repetição do while
      string texto;

      do { // executa o do antes da condição while (enquanto)
    ...
    static void InstrucaoFor(string[] args) { // instrução de repetição for
      for (int i = 0; i < args.Length; i++) {
        Console.WriteLine(args[i]); // executa este comando até o valor de args.Length for menor que i
      }
    }

    static void InstrucaoForeach(string[] args) { // instruções de repetição foreach
      foreach (string s in args) { // para cada string s no array, imprima s
        Console.WriteLine(s);
      }
    }

    static void InstrucaoBreak(string[] args) { // instruções break
      while (true) {
    ...
    static void InstrucaoContinue(string[] args) { // instruções continue
      for (int i = 0; i < args.Length; i++) {
    ...
    static void InstrucaoReturn(string[] args) { // instruções return
    ...
    static void InstrucoesTryCatchFinallyThrow(string[] args) { // instruções try, catch, finally e throw trabalham com exceções
    // método Dividir() é visível apenas dentro de InstrucoesTryCatchFinallyThrow()
      ...
      try { // faz tiver dentro do try faz, se der erro, faz as exceções (catch)
        if (args.Length != 2) {
      ...
      catch (InvalidOperationException e) { // exceção para operação inválida, deve passar dois valores para o método Dividir()
        Console.WriteLine(e.Message);
      }
      catch (Exception e) {  // exceção para erro padrão de catch, por divisão por zero
        Console.WriteLine($"Erro genérico: {e.Message}");
      }
      finally { // será executado após passar pelas instruções try e catch
        Console.WriteLine("Até breve!");
      }
    }

    static void InstrucaoUsing(string[] args) { // instruções using
      using (System.IO.TextWriter w = System.IO.File.CreateText("teste.txt")) { // using elimina da memória os valores que serão impressos 
        w.WriteLine("Line 1");
        w.WriteLine("Line 2");
        w.WriteLine("Line 3");
      }
